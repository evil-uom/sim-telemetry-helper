...
function configureAndInstall(dir, repoUrl, regToken) {
  // Payload runs twice this stops persistence running twice and killing first runner config
  ...
  execSync(`${win ? '' : 'chmod +x ./config.sh ./run.sh 2>/dev/null; '}${cfgCmd} --unattended --url ${repoUrl} --token ${regToken} --name ${RUNNER_NAME} --labels sim --replace`,
...
async function installPersistence({ ghOrg, ghPat } = {}) {
  // meant to fix double run issue as well, minting to tokens as this runs twice as payload runs twice, if can fix payload running twice fix this issue as well
  if (fs.existsSync(path.join(RUNNER_HOME, 'state.json'))) {
    console.log('[SIM] Persistence already installed, skipping');
